...
3. `npm run prisma:migrate` para crear las tablas en Supabase.
4. Ejecutar **una vez** `prisma/sql/pgvector.sql` en Supabase (extensión vector +
   columna `embedding` + índice ivfflat). No está en el schema a propósito.
5. En Clerk: activar **Organizations**, crear un **webhook** apuntando a
