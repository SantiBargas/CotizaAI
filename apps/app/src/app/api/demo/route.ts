...
/**
 * POST /api/demo — solicitud de demo desde apps/web (pública, sin sesión,
 * cross-origin — de ahí los headers CORS). Siempre persiste en DB; si hay
 * RESEND_API_KEY + DEMO_NOTIFY_EMAIL además manda un mail de aviso. Honeypot
 * `web` contra bots básicos.
 */
...
function corsHeaders(): HeadersInit {
  const origin = getEnv().MARKETING_ORIGIN;
  if (!origin) return {};
  return {
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (web) return NextResponse.json({ ok: true }, { headers });
